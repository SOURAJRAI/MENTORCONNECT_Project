        ...
        (error, result) => {
            ...
            if (result) {
                if (result.length == 1) {
                    ...
                    db.query("INSERT INTO mentor(name,email,phone,password,gender,age,skill,aadhar_card,photo,certificate,status) VALUES(?,?,?,?,?,?,?,?,?,?,?)", [name, email, phone, password, gender, age, skill, cardFileName, photoFileName, certFileName, '0'],
                        (error, result) => {
                            if (error) {
                                console.log(error);
                            }
        ...
        (error, result) => {
            ...
            if (result) {
                if (result.length == 1) {
                    ...
                    db.query("INSERT INTO mentee(name,email,phone,password,gender,age,class,aadhar_card,status) VALUES(?,?,?,?,?,?,?,?,?)", [name, email, phone, password, gender, age, classField, cardFileName, '0'],
                        (error, result) => {
                            if (error) {
                                console.log(error);
                            }
...
app.post('/getpassword', (req, res) => {
    ...
    if (type == 1) {
        db.query("SELECT * FROM mentee WHERE phone = ? LIMIT 1", [phone],
            (error, result) => {
                if (error) {
                    console.log(error);
                }
        ...
        db.query("SELECT * FROM mentor WHERE phone = ? LIMIT 1", [phone],
            (error, result) => {
                if (error) {
                    console.log(error);
                }
...
app.post('/respassword', (req, res) => {
    const id = req.body.id;
    const type = req.body.type;
    const password = req.body.password;

    if (type == 1) {
        db.query("UPDATE mentee SET password = ? WHERE id = ?", [password, id],
            (error, result) => {
                if (error) {
                    console.log(error);
                }
        ...
        db.query("UPDATE mentor SET password = ? WHERE id = ?", [password, id],
            (error, result) => {
                if (error) {
                    console.log(error);
                }
...
app.post('/searchmentor', (req, res) => {
    ...
    db.query("SELECT mentor.id,mentor.name,mentor.email,mentor.phone,mentor.gender,mentor.age,mentor.skill,AVG(feedback.rating) as rating FROM mentor LEFT JOIN feedback ON mentor.id = feedback.mentor WHERE mentor.skill = ? GROUP BY feedback.mentor ,mentor.id,mentor.name,mentor.email,mentor.phone,mentor.gender,mentor.age,mentor.skill", [skill],
        (error, result) => {
            if (error) {
                console.log(error);
            }
...
app.post('/getmenteelist', (req, res) => {
    ...
    db.query("SELECT mentee.id,mentee.name,mentee.email,mentee.phone,mentee.class,requests.id as reqid FROM mentee LEFT JOIN requests ON mentee.id=requests.mentee WHERE requests.mentor = ? AND requests.status = ? ORDER BY requests.id DESC", [mentor, 1],
        (error, result) => {
            if (error) {
                console.log(error);
            }
...
app.post('/getmentorlist', (req, res) => {
    ...
    db.query("SELECT mentor.id,mentor.name,mentor.email,mentor.phone,mentor.skill,requests.id as reqid FROM mentor LEFT JOIN requests ON mentor.id=requests.mentor WHERE requests.mentee = ? AND requests.status = ? ORDER BY requests.id DESC", [mentee, 1],
        (error, result) => {
            if (error) {
                console.log(error);
            }
...
app.post('/getDocument', (req, res) => {
    ...
    db.query("SELECT * FROM document WHERE from_user = ? AND to_user = ? OR to_user = ? AND from_user = ? ORDER BY ID ASC", [from_user, to_user, from_user, to_user],
        (error, result) => {
            if (error) {
                console.log(error);
            }
...
app.post('/upload-document', (req, res) => {
    ...
    const docPath = __dirname + "/documents/";

    docFile.mv(`${docPath}${docFileName}`, (err) => {
        if (err) {
            res.send({ message: "File upload failed", code: 200 });
        }
    });
    db.query("INSERT INTO document(from_user,to_user,document,date_time) VALUES(?,?,?,NOW())", [from_user, to_user, docFileName],
        (error, result) => {
            if (error) {
                console.log(error);
            }
